...
$mvisualText3 = $_POST["mvisualText3"];
    ...
    $insert = $manuales -> prepare ("
        INSERT INTO mvisual
        (mvisualName, mvisualText, mvisualText2, mvisualText3, cacId)
        VALUES
        (?,?,?,?,?)
    ");
    ...
    foreach ($_FILES["archivo2"]['tmp_name'] as $key => $tmp_name) {
        ...
        if ($_FILES["archivo2"]["name"][$key]) {
            ...
            $directorio2 = '../mvisual/' . $id . '/'; //Declaramos un  variable con la ruta donde guardaremos los archivos
  ...
  echo "<script> window.location='../c/{$module}.php?m=index2&n=added&cacId={$cacId}'</script>";