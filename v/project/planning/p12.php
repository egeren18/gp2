<?php 
$contadorr = 1;
while($contadorr <= $npregunta){
?>
<li>
    <div class="collapsible-header">
        <table style="width:100%;">
            <tr>
                <td class="blue-grey lighten-2 white-text" style="line-height : 20px;">
                    <?php
                    if ($contadorr >= 1){ echo $mmodelo -> texto1;}
                    if ($contadorr >= 2){ echo $mmodelo -> texto2;}
                    if ($contadorr >= 3){ echo $mmodelo -> texto3;}
                    if ($contadorr >= 4){ echo $mmodelo -> texto4;}
                    if ($contadorr >= 5){ echo $mmodelo -> texto5;}
                    if ($contadorr >= 6){ echo $mmodelo -> texto6;}
                    if ($contadorr >= 7){ echo $mmodelo -> texto7;}
                    if ($contadorr >= 8){ echo $mmodelo -> texto8;}
                    if ($contadorr >= 9){ echo $mmodelo -> texto9;}
                    if ($contadorr >= 10){ echo $mmodelo -> texto10;}
                    if ($contadorr >= 11){ echo $mmodelo -> texto11;}
                    if ($contadorr >= 12){ echo $mmodelo -> texto12;}
                    if ($contadorr >= 13){ echo $mmodelo -> texto13;}
                    if ($contadorr >= 14){ echo $mmodelo -> texto14;}
                    if ($contadorr >= 15){ echo $mmodelo -> texto15;}
                    if ($contadorr >= 16){ echo $mmodelo -> texto16;}
                    if ($contadorr >= 17){ echo $mmodelo -> texto17;}
                    if ($contadorr >= 18){ echo $mmodelo -> texto18;}
                    if ($contadorr >= 19){ echo $mmodelo -> texto19;}
                    if ($contadorr >= 20){ echo $mmodelo -> texto20;}?>
                </td>
                <td style="width: 50px;" class="blue-grey lighten-3 center-align">
                    <i style="margin: 0;" class="material-icons white-text">edit</i>
                </td>
            </tr>
        </table>
    </div>
    <div class="collapsible-body">
        <textarea name="a<?php $contadorr;?>" class="ckeditor"></textarea>
    </div>
</li>
<?php 
$contadorr++;
} ?>
<li>
    <div class="collapsible-header">
        <table style="width:100%;">
            <tr>
                <td class="blue-grey lighten-2 white-text" style="line-height : 20px;">
                 Materialidad
                </td>
                <td style="width: 50px;" class="blue-grey lighten-3 center-align">
                    <i style="margin: 0;" class="material-icons white-text">edit</i>
                </td>
            </tr>
        </table>
    </div>
    <div class="collapsible-body">
        
        
  
<table class="white">
    
<tr class="grey lighten-3 blue-grey-text text-darken-1">
<th id="border-white" style="width:30%;" >Punto de referencia:</td>
<th id="border-white"  style="width:25%;">Empresas con beneficios normales</td>
<th id="border-white" ></td>
</tr>


<tr>
<td id="border-grey">Beneficios operaciones continuas antes de impuestos:</td>
<td id="border-grey">
    <div class="input-field">
        <input style="text-align: right ;" type="number" name="riesgoN" class="form-control" required="">
    </div>
</td>
<td id="border-grey">
    <button type="submit" class="btn blue waves-effect tooltipped" data-position="top" data-tooltip="Guardar">
                                    <i class="material-icons">save</i>
    </button>
</td>    
</tr>


</table>

<table class="white">
    
<tr class="grey lighten-3 blue-grey-text text-darken-1">
<th id="border-white" style="width:30%;"  colspan="2">Escoger medicion empirica:</td>
<th id="border-white"  style="width:25%;"></td>
<th id="border-white" ></td>
</tr>


<tr>
<td id="border-grey">Tramo 5% - 10%:</td>


<td id="border-grey" style="width:10%;">
    <div class="input-field">
   <select>
      <option value="" disabled selected>Tramo</option>
      <option value="5">5</option>
      <option value="6">6</option>
      <option value="7">7</option>
      <option value="8">8</option>
      <option value="9">9</option>
      <option value="10">10</option>
    </select>
    </div>
</td>



<td id="border-grey" style="width:15%;">
    <div class="input-field">
        <input style="text-align: right ;" type="number" name="riesgoN" class="form-control" required="">
    </div>
</td>

<td id="border-grey">
    <button type="submit" class="btn blue waves-effect tooltipped" data-position="top" data-tooltip="Guardar">
                                    <i class="material-icons">save</i>
    </button>
</td>    
</tr>

<tr>
<td id="border-grey" colspan="2">Importancia relativa seleccionada:</td>
<td id="border-grey">
    <div class="input-field">
        <input style="text-align: right ;" type="number" name="riesgoN" class="form-control" required="">
    </div>
</td>
<td id="border-grey">
    <button type="submit" class="btn blue waves-effect tooltipped" data-position="top" data-tooltip="Guardar">
                                    <i class="material-icons">save</i>
    </button>
</td>    
</tr>

</table>


<table class="white">
    
<tr class="grey lighten-3 blue-grey-text text-darken-1">
<th id="border-white" style="width:30%;"  colspan="2">Ajustar la importancia relativa global</td>
<th id="border-white"  style="width:25%;"></td>
<th id="border-white" ></td>
</tr>


<tr>
<td id="border-grey">Recorte</td>


<td id="border-grey" style="width:10%;">
    <div class="input-field">
 <input style="text-align: right ;" type="number" name="riesgoN" class="form-control" required="">
    </div>
</td>



<td id="border-grey" style="width:15%;">
    <div class="input-field">
        <input style="text-align: right ;" type="number" name="riesgoN" class="form-control" required="">
    </div>
</td>

<td id="border-grey">
    <button type="submit" class="btn blue waves-effect tooltipped" data-position="top" data-tooltip="Guardar">
                                    <i class="material-icons">save</i>
    </button>
</td>    
</tr>

<tr>
<td id="border-grey" colspan="2">Importancia relativa seleccionada (ajustada):</td>
<td id="border-grey">
    <div class="input-field">
        <input style="text-align: right ;" type="number" name="riesgoN" class="form-control" required="">
    </div>
</td>
<td id="border-grey">
    <button type="submit" class="btn blue waves-effect tooltipped" data-position="top" data-tooltip="Guardar">
                                    <i class="material-icons">save</i>
    </button>
</td>    
</tr>

</table>

<table class="white">
    
<tr class="grey lighten-3 blue-grey-text text-darken-1">
<th id="border-white"  style="width:33%;" colspan="2">Nivel de minimis registro de incorrecciones (opcional)</td>
<th id="border-white"  style="width:22%;"></td>
<th id="border-white" ></td>
</tr>


<tr>
<td id="border-grey">Nivel minimis registro de incorrecciones</td>


<td id="border-grey" style="width:10%;">
    <div class="input-field">
 <input style="text-align: right ;" type="number" name="riesgoN" class="form-control" required="">
    </div>
</td>



<td id="border-grey" style="width:15%;">
    <div class="input-field">
        <input style="text-align: right ;" type="number" name="riesgoN" class="form-control" required="">
    </div>
</td>

<td id="border-grey">
    <button type="submit" class="btn blue waves-effect tooltipped" data-position="top" data-tooltip="Guardar">
                                    <i class="material-icons">save</i>
    </button>
</td>    
</tr>

<tr>
<td id="border-grey" colspan="2">Nivel minimis registro de incorrecciones:</td>
<td id="border-grey">
    <div class="input-field">
        <input style="text-align: right ;" type="number" name="riesgoN" class="form-control" required="">
    </div>
</td>
<td id="border-grey">
    <button type="submit" class="btn blue waves-effect tooltipped" data-position="top" data-tooltip="Guardar">
                                    <i class="material-icons">save</i>
    </button>
</td>    
</tr>

</table>
    
    
    </div>
</li>