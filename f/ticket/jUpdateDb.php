...
$nombreA = $_SESSION["userFullName"];
$ticketTextA = $_POST["ticketTextA"];

$check = $_POST["checkterminado"] == true ? 1 : 0;


 if ($check == 1) {
     include 'Ucompletado.php';
} 
else {
// update
  ...
  foreach ($_FILES["archivo2"]['tmp_name'] as $key => $tmp_name) {
      ...
      if ($_FILES["archivo2"]["name"][$key]) {
          ...
          $directorio2 = '../ticketA/' . $id . '/'; //Declaramos un  variable con la ruta donde guardaremos los archivos
...
echo "<script> window.location='../c/{$module}.php?m=index&n=updated&d=1'</script>";
} 
