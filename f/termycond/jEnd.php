...
$update = $connection -> prepare("UPDATE termycond SET statusId = ? WHERE termycondId = ? AND mId = 1");
$update -> bind_param("ii", $statusId, $termycondId);
$update -> execute();

$update = $connection -> prepare("UPDATE termycond SET statusId = ? WHERE termycondId = ? AND mId = 2");
$update -> bind_param("ii", $statusId, $termycondId);
$update -> execute();

$update = $connection -> prepare("UPDATE termycond SET statusId = ? WHERE termycondId = ? AND mId = 3");
$update -> bind_param("ii", $statusId, $termycondId);
$update -> execute();
