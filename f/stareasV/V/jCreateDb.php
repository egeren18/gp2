...
// date
    ...
    $insert = $master -> prepare ("
        INSERT INTO stareas
        (stareasName, stareasText, testimado, ptareaId, userId )
        VALUES
        (?,?,?,?,?)
    ");
    ...
    $insert -> execute();
    $id = 0;
 $stareas = mysqli_query(
            $master, "SELECT * FROM stareas
             order by stareasId  desc"
    );
     while ($r_stareas = mysqli_fetch_array($stareas)) {
        $id = $id + 1; 
    }
