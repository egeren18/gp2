                                            ...
                                            <table id="example" class="display nowrap" style="width:100%">
                                                ...
                                                <tbody>
                                                    ...
                                                    <tr>
                                                        ...
                                                        <?php if ($_SESSION["userAdmin"] == 4) { ?>
                                                        <td class="center">
                                                            <a target="_blank" href='../plugins/pdf/doc/userAccess.php?id=<?php echo $user -> userId; ?>' class='tooltipped' data-position='left' data-tooltip='Ver'><i class='material-icons blue-text'>search</i></a>
                                                        </td>
                                                        <?php } else { ?>
                                                        <td>
                                                            <?php
                                                            $_access = mysqli_query($master, "SELECT * FROM access WHERE userId = '" . $user -> userId . "'");
                                                            $access = $_access -> fetch_object();
                                                            $userId = $access -> userId;
                                                            if ($access -> userId  == $user -> userId) {
                                                                echo $link = $allow -> jUpdate == 1 ? "<a href='../c/access.php?m=update&uid={$user -> userId}&sid={$user -> subscriberId}' class='tooltipped right' data-position='left' data-tooltip='Modificar'><i class='material-icons blue-text'>edit</i></a>" : "<a href='javascript: void(0)'><i class='material-icons grey-text'>edit</i></a>";
                                                            }
                                                            else {
                                                                echo $link = $allow -> jCreate == 1 ? "<a href='../c/access.php?m=create&uid={$user -> userId}&sid={$user -> subscriberId}' class='tooltipped right' data-position='left' data-tooltip='Asignar'><i class='material-icons blue-text'>add</i></a>" : "<a href='javascript: void(0)'><i class='material-icons grey-text'>add</i></a>";
                                                            }
                                                            ?>
                                                        </td>
                                                        <?php } ?>
