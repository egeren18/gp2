...
// date
    ...
    $insert = $master -> prepare ("
        INSERT INTO ticket
        (ticketName, ticketText, nombre, subs, subsId, departmentId)
        VALUES
        (?,?,?,?,?,?)
    ");
    ...
    $insert -> bind_param (
        "ssssii",
        $titulo, $ticketText, $nombre, $subs, $subsId, $grupoId
    );
    ...
    $id = 0;
 $ticket = mysqli_query(
            $master, "SELECT * FROM ticket
             order by ticketId  desc"
    );
     while ($r_ticket = mysqli_fetch_array($ticket)) {
        $id = $id + 1; 
    }
    ...
    foreach ($_FILES["archivo2"]['tmp_name'] as $key => $tmp_name) {
        ...
        if ($_FILES["archivo2"]["name"][$key]) {
            ...
            $directorio2 = '../ticket/' . $id . '/'; //Declaramos un  variable con la ruta donde guardaremos los archivos
