...
$pdf = new PDF(); $pdf -> AddPage(); $pdf -> AliasNbPages(); $pdf -> AddFont('Ubuntu', '', 'Ubuntu-Light.php'); $pdf -> SetFont('Ubuntu', '', 9); while ($item = $_item -> fetch_object()) { $pdf -> Cell(40, 6, '- ' . utf8_decode($item -> countryName)); $pdf -> Ln(); } $pdf -> Output('I', 'paises.pdf', '');
